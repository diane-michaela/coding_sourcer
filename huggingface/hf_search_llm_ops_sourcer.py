"""
Hugging Face Hub Sourcer — Senior ML Engineer, Search & LLM Ops (RavenPack trial)

Standalone script, separate from huggingface_retrieval_sourcer_excel.py and
hf_ml_engineer_ai_specialist_sourcer.py (does not share code or config with either).

Built for: https://ravenpack.teamtailor.com/jobs/8186536-senior-ml-engineer-search-llm-ops
Core JD signals: ranking/retrieval models for search relevance, LLM fine-tuning/distillation
for domain-specific (finance) expertise, semantic embeddings (binary, matryoshka, late
interaction), personalized recommender systems, vector databases, PyTorch/TensorFlow/
scikit-learn/Hugging Face, bonus: financial services + MLOps/cloud.

What it does:
- Searches HF Hub models + datasets + Spaces for JD keywords (reranker, cross-encoder,
  bi-encoder, dense retrieval, semantic search, colbert, splade, matryoshka embedding,
  binary embedding, late interaction, recommender system, learning to rank, knowledge
  distillation, vector database)
...
Usage:
    python hf_search_llm_ops_sourcer.py --dry-run          # free preview, no Claude spend
...
JD_TITLE = "Senior ML Engineer — Search & LLM Ops (RavenPack)"
JD_KEYWORDS = [
    "reranker",
    "cross-encoder",
    "bi-encoder",
...
OUTPUT_BASENAME = "hf_search_llm_ops_candidates"
BATCH_ID_FILE = Path(__file__).with_name(".search_llm_ops_last_batch_id")
CANDIDATES_CACHE_FILE = Path(__file__).with_name(".search_llm_ops_candidates_cache.json")
...
{JD_TITLE}
Core signals we're looking for: ranking algorithms and retrieval models for search relevance, LLM \
fine-tuning/distillation for domain-specific expertise, semantic embedding techniques (binary embeddings, \
matryoshka embeddings, late interaction models), personalized recommender systems, hands-on vector database \
experience, and PyTorch/TensorFlow/scikit-learn/Hugging Face proficiency. Bonus signals: financial services \
industry experience, MLOps tools, cloud platform expertise.
...
Signal calibration — a published Space (an interactive demo) is stronger evidence of hands-on building than an \
uploaded model or dataset alone, since it proves the person wired something together end-to-end rather than just \
sharing a checkpoint; weigh a candidate with one or more Spaces among their matched repos above one with the same \
repo count but no Spaces. Likewise weigh recent activity (matched repos touched in the last few months) above a \
single old repo with nothing since, and treat a bio, an affiliated org, or multiple matched repos as reinforcing \
signals — not as prerequisites. A single old, unmaintained repo with a matching tag but no other signal (no bio, \
no other activity, 12+ months stale) is a weak match even if the tag looks relevant on paper; say so plainly in \
score_reasons rather than inflating the score off tag overlap alone.
...
try:
    from token_hf import HF_TOKEN as FILE_TOKEN  # optional legacy fallback shared with the other HF scripts
except Exception:
    FILE_TOKEN = ""
...
def enrich_candidates(candidates: dict) -> list:
    ...
    for username, cand in candidates.items():
        ...
        kind_counts = Counter(r["kind"] for r in cand["repos"])
        # Spaces are working demos, not just uploaded checkpoints — call the count out
        # separately so the scoring model can weigh "actually builds things" higher.
        kind_breakdown = ", ".join(
            f"{kind_counts[k]} {kind_label[k]}{'s' if kind_counts[k] != 1 else ''}"
            for k in ("spaces", "models", "datasets") if kind_counts.get(k)
        )
        prolific_signal = (
            f"{len(cand['repos'])} matching repo(s) [{kind_breakdown}] "
            f"({total_likes} likes, {total_downloads} downloads) · {hf_totals}"
        )
