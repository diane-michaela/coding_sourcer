"""
GitHub repo sourcer for Forward Deployed Engineer / Product Engineer / Design Engineer profiles.

Targets React + TypeScript + design-system / Figma / AI-assisted workflows.
Filters owners to France (extendable to nearby European countries).
Writes a local Excel file.

Run manually: python fde_react_sourcer.py
...
Env:
  GITHUB_TOKEN            (required for decent rate limits)
...
# Multiple focused queries work much better than one heavy AND.
BASE_QUERIES = [
    # Tight design-system + testing signal
    'storybook cypress tailwindcss in:readme,description language:TypeScript stars:>1',

    # Classic design-system builders
    '"design system" (react OR "react.js") in:readme,description language:TypeScript stars:>3',

    # Explicit Figma → code signal (high value when present)
    '(figma OR "from figma" OR "figma to" OR "figma to react") (react OR typescript) in:readme,description language:TypeScript stars:>1',

    # Claude / Cursor / AI-assisted design-to-code
    '(claude OR "claude code" OR cursor) (react OR typescript) (figma OR "design system" OR tailwind OR storybook) in:readme,description language:TypeScript stars:>1',

    # Full-stack product signal (front + real backend)
    'react (redis OR postgresql OR supabase OR prisma) in:readme,description language:TypeScript stars:>4',

    # Persona language (noisier, but useful combined with stack)
    '("product engineer" OR "design engineer" OR "founding engineer") (react OR typescript) in:readme,description language:TypeScript stars:>1',
]

# France only for now. Uncomment the Europe set later if you need more volume.
TARGET_COUNTRIES = {"France"}
# TARGET_COUNTRIES = {
#     "France", "Belgium", "Switzerland", "Germany", "Netherlands",
#     "Spain", "Portugal", "Italy", "United Kingdom", "Ireland",
#     "Sweden", "Norway", "Denmark", "Finland", "Austria", "Poland"
# }

# Keep owners with empty location (common on GitHub). Set to False once France volume is good.
KEEP_UNKNOWN_LOCATION = True

STATE_FILE = Path(__file__).with_name("state_fde_grok_react.json")
...
DEFAULT_XLSX = "github_fde_grok_candidates.xlsx"
...
SESSION.headers.update({
    ...
    "User-Agent": "fde-react-sourcer/1.1",
})
if ENV_TOKEN:
...
def geocode_nominatim(raw: str) -> dict:
    ...
    try:
        ...
        geolocator = Nominatim(user_agent="fde-react-sourcer/1.1", timeout=10)
...
def _passes_location_filter(row: dict) -> bool:
    country = (row.get("owner_country") or "").strip()
    if not country:
...
def main():
    ...
    print("Target countries:", TARGET_COUNTRIES, "| keep unknown location:", KEEP_UNKNOWN_LOCATION)
    ...
    kept_rows = [r for r in user_owned if _passes_location_filter(r)]
    print(f"After location filter ({TARGET_COUNTRIES}, keep_unknown={KEEP_UNKNOWN_LOCATION}): {len(kept_rows)}")
    ...
    save_state(state)
    save_geo_cache()
    print("\nDone. Review the Excel before deciding on next steps.")
