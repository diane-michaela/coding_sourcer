"""
GitHub repo sourcer (NLP/LLM production engineering) + owner enrichment + location geocoding/normalization.

- Searches GitHub repos by keyword clusters (inference, quantization, alignment, PEFT/LoRA, vector DB)
  with one query per keyword; excludes RAG/agent-related terms.
...
# Keyword clusters: one query per keyword (no single large OR query)
...
# Exclude RAG/agent repos (do not include vendor names from vector_database)
EXCLUDE_TERMS = [
    "RAG",
    "retrieval augmented",
    "retrieval-augmented",
...
def build_cluster_query(keyword: str, start: datetime, end: datetime, time_filter: str) -> str:
    """Build GitHub search query for one keyword with date range and negative filters.
    time_filter is 'created' or 'pushed'."""
    start_date = start.date().isoformat()
    end_date = end.date().isoformat()
    base = f'"{keyword}" in:readme in:description {time_filter}:{start_date}..{end_date} fork:false'
    negatives = " ".join([f'-"{t}"' if " " in t else f'-{t}' for t in EXCLUDE_TERMS])
...
def _build_row_from_repo(
    ...
    """Build one sheet row from a GitHub repo with traceability: skill_cluster, keyword_matched, query, scan_type."""
...
def main():
    ...
    def run_scan(scan_type: str) -> None:
        nonlocal total_seen
        for cluster_name, keywords in QUERY_CLUSTERS.items():
            if len(all_rows) >= MAX_REPOS:
                break
            for keyword in keywords:
                if len(all_rows) >= MAX_REPOS:
                    break
                q = build_cluster_query(keyword, window_start, window_end, scan_type)
                print(f"[{scan_type}] {cluster_name} / {keyword!r}: {q[:80]}...")
                seen_this_query = 0
                for repo in search_repositories(q):
                    row = _build_row_from_repo(repo, cluster_name, keyword, q, scan_type)
                    ...
                    if total_seen % 50 == 0:
                        save_geo_cache()
                        print(f"Progress: {total_seen} fetches, {len(all_rows)} unique repos")
                    if seen_this_query >= MAX_REPOS_PER_QUERY:
                        break
                    if len(all_rows) >= MAX_REPOS:
                        break

    # Created scan (one query per keyword per cluster)
    run_scan("created")
