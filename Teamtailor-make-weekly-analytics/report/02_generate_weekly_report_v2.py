"""
Make module 5 ("Generate report") — code:ExecuteCode, Python — V2
Scenario: TeamTailor-report V2 (diane) — new scenario, People folder in Make.

Local, standalone copy of the code to paste into Scenario 1's compute module.
Same TeamTailor-fetch-and-compute logic as the live v1 script
(../02_generate_weekly_report.py), with three changes agreed for V2:

  1. STAGE_ORDER gets two new stages: "Reference check" and "Offered"
     (replacing "Offer" — assumed renamed, not added alongside it. If
     TeamTailor actually has both "Offer" and "Offered" as distinct stages,
     put both back in STAGE_ORDER and adjust the conversion/pipeline lines
     below accordingly).
  2. Candidate names are now surfaced for Reference check, Offered, and
     Hired too (previously only the three interview stages showed names).
  3. `result` now includes `hired_candidates` — needed by Scenario 2
     (Final Summary) to name who got hired, and by module "DS2: Add a
     Record" to persist it into the Weekly Snapshots data store.

To push into Make: re-hardcode job_id/channel as {{40.job_id}} / {{40.channel}}
pills (or {{<DS1 search step>.job_id}} once module 1 is replaced by a Data
Store Search Records module — see ../V2_ROLLUP_SPEC.md), and keep API_TOKEN
as a Make Custom Variable ({{API_TOKEN}}) rather than hardcoded.
"""
import json
import os
...
# Stages where we show candidate names inline, not just a count.
NAME_STAGES = [
    "Job-fit interview",
    "Use-case Interview",
    "Cultural-fit interview",
...
# Pipeline snapshot lines — inline names for stages in NAME_STAGES
...
result = {
    ...
    # New in V2 — feeds DS2 (Weekly Snapshots) and Scenario 2's thank-you message.
